#!/usr/bin/env python3
"""
Distributed image encoding pipeline for BLIP3o long-caption pretraining data.

Launch example (2 GPUs):
  torchrun --nproc_per_node=2 ddp_blip3o_image_encoder.py \
...
Requirements:
  ...
  - NCCL-capable multi-GPU environment
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DDP image encoder for BLIP3o streaming dataset")
...
def build_shared_output_dir(rank: int, output_dir_arg: str) -> str:
    if output_dir_arg:
        if rank == 0:
            os.makedirs(output_dir_arg, exist_ok=True)
        shared = [output_dir_arg]
    else:
        shared = [tempfile.mkdtemp(prefix="blip3o_patch_tokens_") if rank == 0 else ""]
...
def main() -> None:
    ...
    if world_size != 2 and rank == 0:
        print(f"Warning: expected world_size=2 for 2xT4 topology, got world_size={world_size}")
    ...
    try:
        ...
        ds = ds.shard(num_shards=2, index=local_rank)
        ...
        output_dir = build_shared_output_dir(rank, args.output_dir)
        hub_api = None
        ...
        with torch.inference_mode():
            for batch in loader:
                ...
                patch_tokens = infer_patch_tokens(ddp_model.module, pixel_values)
        ...
        processed_tensor = torch.tensor([processed], device=device, dtype=torch.long)
        ...
        if rank == 0:
            ...
            # - Hugging Face Hub is file/commit oriented, not row-wise streaming.
            # - Best practice is to upload generated parquet chunks in batches.
            # - Example batched upload sketch:
            #
            # from huggingface_hub import HfApi as HuggingFaceHubAPI
            # api = HuggingFaceHubAPI(token=os.environ["HF_TOKEN"])
            # repo_id = "your-username/blip3o-tipsv2-patchtokens"
            ...
            #     batch = parquet_files[start : start + batch_size]
            #     for local_file in batch:
            #         path_in_repo = f"parquet/{os.path.basename(local_file)}"
            #         api.upload_file(
            #             path_or_fileobj=local_file,
            #             path_in_repo=path_in_repo,
            #             repo_id=repo_id,
